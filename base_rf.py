...
def generate_baseClassifiers(features, data, external, base_path):
    # hyperparameters
    ...
    paras = [l for l in itertools.product(n_estimators, max_depth, min_samples_split, seeds, weight)]
    print(len(paras))
    para = paras[int(var)]
    print(para)


    X_org,  y_org = data[features],  data['Ames\nResult']
    X, X_val, y, y_val = train_test_split(X_org,  y_org, test_size=0.2, stratify=y_org, random_state=7)

    ###external dataset
    X_test, y_test = external[features], external['label']
    ...
    for i in range(20):
        ...
        for train_index, validation_index in skf.split(X, y):
            ...
            ### define column name 
            col_name = 'rf_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var+'_estimator_'+str(para[0])+'_depth_'+str(para[1])+'_minSample_'+str(para[2])+'_random_'+str(para[3])+'_weight_'+str(para[4])
            col_name2 = 'rf_'+'_paras_'+var

            ### create and fit model
            clf = RandomForestClassifier(n_estimators=para[0], max_depth=para[1],  min_samples_split=para[2], random_state=para[3], class_weight =para[4], bootstrap = True, max_features='log2')
