...
def combine_test_probabilities(base_path, mcc, probability_path, name): 
    
    ### the path for five different base classifiers
    knn_base_path = base_path + '/knn/test_class' 
    lr_base_path = base_path +  '/lr/test_class'
    svm_base_path = base_path + '/svm/test_class'
    rf_base_path = base_path + '/rf/test_class'
    xgboost_base_path = base_path + '/xgboost/test_class'
    ...
    tmp = pd.read_csv(join(knn_base_path, 'test_knn_paras_0.csv'))
    knn = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(lr_base_path, 'test_lr_paras_22.csv'))
    lr = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(svm_base_path, 'test_svm_paras_19.csv'))
    svm = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(rf_base_path, 'test_rf__paras_148.csv'))
    rf = tmp[['id', 'y_true']]
    ...
    tmp = pd.read_csv(join(xgboost_base_path, 'test_xgboost_paras_151.csv'))
    ...
    data.to_csv(join(probability_path+'/test_probabilities_' + name + '.csv'))

    print('Combining test predictions is completed')
