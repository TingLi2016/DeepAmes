...
def generate_baseClassifiers(features, data, external, base_path):
    ...
    gammas = [1000, 100, 10, 1, 0.1, 0.001, 0.0001]
    gammas.append('scale')
    ...
    for i in range(20):
        ...
        for train_index, validation_index in skf.split(X, y):
            ...
            ### define column name 
            col_name = 'svm_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var+'_kernel_'+para[0]+'_C_'+str(para[1])+'_gamma_'+str(para[2])
            col_name2 = 'svm_'+'paras_'+var

            ### create and fit model
            clf = SVC(kernel=para[0], C=para[1], gamma=para[2], probability=True, class_weight = 'balanced', random_state=1)#
