...
def generate_baseClassifiers(features, data, external, base_path):
    ...
    paras = [l for l in itertools.product(Cs, weights, solvers)]
    print(len(paras))
    para = paras[int(var)]
    print(para)
    ...
    for i in range(20):
        ...
        for train_index, validation_index in skf.split(X, y):
            ...
            ### define column name
            col_name = 'lr_'+'seed_'+str(i)+'_skf_'+str(j)+'_paras_'+var+'_C_'+str(para[0])+'_weight_'+str(para[1])+'_solver_'+str(para[2])
            col_name2 = 'lr_'+'paras_'+var

            ###create classifier
            clf = LogisticRegression(C=para[0], class_weight=para[1], solver=para[2], max_iter=300, random_state=1)
